...
static const char *CA_CORE_TOOL_PROMPT =
    ...
    "- git_status and git_diff are read-only; this runtime does not provide git_commit in the current phase.\n"
    "- Do not request dangerous, interactive, chained, background, network download, or workspace-bypassing commands.\n"
    "- Do not use execute_command for git reset, git clean, git checkout, git push, or other destructive Git operations.\n"
    ...
    "- Never claim code has been committed unless a future git_commit tool reports success=true.\n"
